...
namespace Microsoft.Msagl.Core.ProjectionSolver
{
    ...
    internal class Qpsc
    {
        ...
        {
#if VERBOSE
            Debug.WriteLine("Q and b (before scaling)...");
            DumpMatrix();
...
#if VERBOSE
            Debug.WriteLine("Qpsc.VariablesComplete Variable scales and values");
            Debug.WriteLine(" ordinal     origDesired     desired     scale       unscaled actualpos      scaled actualpos");
...
#if VERBOSE
            Debug.WriteLine("Q' and b' (after scaling)...");
            DumpMatrix();
            DumpVector("WiDi", vectorWiDi);
#endif // VERBOSE
        ...
        internal bool PreProject()
        {
            ...
            {
#if VERBOSE
                Debug.WriteLine("Previous and scale-munged ActualPos:");
                Debug.WriteLine(" Ordinal   origDesired desired   unscaledPos     scaledPos");
                foreach (var qvar in vectorQpscVars)
                {
                    Debug.Assert(qvar.Variable.ActualPos == vectorCurY[qvar.Variable.Ordinal], "FirstProject: ActualPos != CurY");
                    Debug.WriteLine("    {0} {1} {2} {3} {4}", qvar.Variable.Ordinal,
                                qvar.OrigDesiredPos, qvar.Variable.DesiredPos,
            ...
            {
#if VERBOSE
                Debug.WriteLine("Converging due to zero-gradient");
#endif // VERBOSE
                return false;
            }
            double alpha = alphaNumerator / alphaDenominator;
#if VERBOSE
            Debug.WriteLine("Alpha = {0} (num {1}, den {2})", alpha, alphaNumerator, alphaDenominator);
#endif // VERBOSE
...
#if VERBOSE
            Debug.WriteLine("NormAlphaG = {0}", this.normAlphaG);
#endif // VERBOSE
#endif // VERIFY || VERBOSE

            // Update d(esiredpos) = y - alpha*g
#if VERBOSE
            Debug.WriteLine("PreProject gradient-stepped desired positions:");
#endif // VERBOSE
            ...
            for (var ii = 0; ii < this.vectorCurY.Length; ++ii)
            {
                ...
                Debug.WriteLine("{0} curY {1} orig desiredpos = {2}, current desiredpos = {3}, gradient = {4}",
        ...
        internal bool PostProject()
        {
            ...
            //
#if VERBOSE
            Debug.WriteLine("PostProject current scaled and unscaled ActualPos:");
#endif // VERBOSE
            foreach (var qvar in this.vectorQpscVars)
            {
                this.vectorCurY[qvar.Variable.Ordinal] = qvar.Variable.ActualPos;
#if VERBOSE
                Debug.WriteLine("    {0} {1} {2}", qvar.Variable.Ordinal, qvar.Variable.ActualPos, qvar.Variable.ActualPos * qvar.Variable.Scale);
#endif // VERBOSE
            }
...
#if VERBOSE
            Debug.WriteLine("PostProject variable deltas:");
#endif // VERBOSE
...
#if VERBOSE
                Debug.WriteLine("  {0} {1}", ii, vectorCurY[ii]);
#endif // VERBOSE
            }
            normDeltaY = Math.Sqrt(normDeltaY);
#if VERBOSE
            Debug.WriteLine("NormAlphaG = {0}; normDeltaY = {1}", this.normAlphaG, normDeltaY);
#endif // VERBOSE
            ...
            if (0.0 == betaNumerator) {
                Debug.WriteLine("  Beta-numerator is zero");
            }
#endif // VERBOSE
            if (0.0 != betaNumerator)
            {
...
#if VERBOSE
                Debug.WriteLine("Beta = {0} (num {1}, den {2})", beta, betaNumerator, betaDenominator);
#endif // VERBOSE
                if (beta > 1.0)
                {
                ...
                {
#if VERBOSE
                    Debug.WriteLine("  Resetting negative Beta to zero");
#endif // VERBOSE
...
#if VERBOSE
            Debug.WriteLine("PostProject variable positions (scaled, unscaled) adjusted for Beta:");
            for (int ii = 0; ii < vectorCurY.Length; ++ii) {
                Debug.WriteLine("  {0} {1} {2}", ii, vectorCurY[ii], vectorCurY[ii] * vectorQpscVars[ii].Variable.Scale);
            }
#endif // VERBOSE
...
#if VERBOSE
                    Debug.WriteLine("var {0} block {1} blockscale {2} blockrefpos {3} ActualPos {4}", qvar.Variable.Ordinal, qvar.Variable.Block.Id,
        ...
        private bool HasConverged()
        {
            ...
            Debug.WriteLine("Iteration function value: {0}", dblTestFuncValue);
#endif // VERBOSE
            ...
            if (!this.isFirstProjectCall)
            {
                ...
                Debug.WriteLine("|Xprev - xCur|: {0}", dblDeltaNorm);
#endif // VERBOSE
...
#if VERBOSE
                Debug.WriteLine("  dblPrevFunctionValue {0:F5}, currentFunctionValue {1:F5}, dblDiff {2}, dblQuotient {3}",
                ...
                {
#if VERBOSE
                    Debug.WriteLine("  Terminating due to function value change within convergence epsilon");
#endif // VERBOSE
                    fConverged = true;
                }
        ...
        void DumpMatrix() {
            ...
            Debug.WriteLine("A matrix: {0} cells (average {1:F2} per row)", activeCells, (double)activeCells / matrixQ.Length);
        }
