...
namespace Microsoft.Msagl.Core.ProjectionSolver
{
    ...
    internal class Block
    {
        ...
        double Recursive_DfDv(Variable varToEval, Constraint currentConstraint, int level) {
...
#if VERBOSE
            Debug.WriteLine("Recursive_DfDv level {0} initial dfdv = {1}", level, dfdv);
            Debug.WriteLine("    varToEval = " + varToEval);
            if (0 != level) {
                Debug.WriteLine("    currentConstraint  = " + currentConstraint);
                Debug.WriteLine("    " + ((varToEval == currentConstraint.Right) ? "(LToR)" : "RToL"));
            }
#endif // VERBOSE
...
#if VERBOSE
                    Debug.WriteLine("Recursive_DfDv level {0} LToR cst {1} Lm = {2}, .Left scale = {3}, dfdv = {4}",
            ...
            foreach (var constraint in varToEval.RightConstraints) {
                if (constraint.IsActive && (constraint != currentConstraint)) {
                    ...
                    dfdv -= constraint.Lagrangian;
#if VERBOSE
                    Debug.WriteLine("Recursive_DfDv level {0} RToL cst {1} Lm = {2}, .Right scale = {3}, dfdv = {4}",
...
#if VERBOSE
            Debug.WriteLine("Recursive_DfDv level {0} final dfdv = {1}", level, dfdv);
#endif // VERBOSE
            return dfdv;
        }
...
#endif // DEBUG
#if VERBOSE
            Debug.WriteLine("ComputeDfDv initialVarToEval: [{0}]", initialVarToEval);
#endif // VERBOSE
...
#if VERBOSE
                Debug.WriteLine("ComputeDfDv peeking at node {0}: varDoneEval {1}, {2}Constraint: {3}",
...
#if VERBOSE
                Debug.WriteLine("ComputeDfDv: Node has no children or all have been processed");
#endif // VERBOSE
...
#if VERBOSE
            Debug.WriteLine("ComputeDfDv result: {0:F5}", dummyConstraint.Lagrangian);
#endif // VERBOSE
        ...
        {
#if VERBOSE
            Debug.WriteLine("  ComputeDfDv depth {0} evaluating {1}Constraint: {2}", 
            ...
            if (null != this.pathTargetVariable) {
                Debug.WriteLine("    {0} dead end: {1}", node.IsLeftToRight ? "LtoR" : "RtoL", node.ConstraintToEval);
            }
#endif // VERBOSE
...
#if VERBOSE
            Debug.WriteLine("  ComputeDfDv: incremental {0} dfdv = {1:F5}", node.IsLeftToRight ? "LToR" : "RToL", dfdv);
            Debug.WriteLine("    Constraint {0} Lagrangian: {1:F5} (scale when {2} parent = {3:F5})",
            ...
            Debug.WriteLine("        Parent {0} Lagrangian: {1:F5}",
        ...
        void Debug_CheckForViolatedActiveConstraint(Constraint constraint)
        {
            ...
            {
#if VERBOSE
                Debug.WriteLine("Violated active constraint encountered: {0}", constraint);
#endif // VERBOSE
        ...
        {
#if VERBOSE
            Debug.WriteLine("ComputeDfDv directly processing the following leaf constraint:");
#endif // VERBOSE
        ...
        {
#if VERBOSE
            Debug.WriteLine("ComputeDfDv depth {0} pushing non-leaf {1}Constraint: {2}",
                    node.Depth, node.IsLeftToRight ? "Left" : "Right", node.ConstraintToEval);
#endif // VERBOSE
        ...
        void DumpPath(string strPrefix, Constraint minLagrangianConstraint)
        {
            ...
            {
                Debug.WriteLine("{0} Dir={1}{2}", pathItem.Constraint, pathItem.IsForward ? "fwd" : "bwd",
        ...
        static void DumpSplitConstraint(string strPrefix, Constraint minLagrangianConstraint)
        {
            ...
            Debug.WriteLine(minLagrangianConstraint);
        }
#endif // VERBOSE
        ...
        internal void Expand(Constraint violatedConstraint)
        {
...
#if VERBOSE
            Debug.WriteLine("Before Block.Expand ComputeDfDv: {0}", this);
#endif // VERBOSE

            ComputeDfDv(violatedConstraint.Left);

#if VERBOSE
            Debug.WriteLine("After Block.Expand ComputeDfDv: {0}", this);
            ...
            if (this.constraintPath.Count > 0)
            {
                ...
                {
#if VERBOSE
                    Debug.WriteLine("ConstraintPath: {0} ({1})", pathItem.Constraint, pathItem.IsForward ? "forward" : "backward");
#endif // VERBOSE
...
#if VERBOSE
                Debug.WriteLine("  -- Expand: No forward non-equality edge (minLagrangianConstraint) found, therefore the constraint is unsatisfiable -- ");
                Debug.WriteLine("     Unsatisfiable constraint: {0}", violatedConstraint);
#endif // VERBOSE
                return;
            }
...
#if VERBOSE
            Debug.WriteLine("Block.Expand result: {0}", this);
            DumpState(null /* no prefix */);
#endif // VERBOSE
        } // end Expand()
...
)
        {
            ...
            // and the gap grown.
#if VERBOSE
            Debug.WriteLine("Before Block.Split ComputeDfDv: {0}", this);
#endif // VERBOSE

            ComputeDfDv(this.Variables[0]);

#if VERBOSE
            Debug.WriteLine("After Block.Split ComputeDfDv: {0}", this);
            DumpState(null /* no prefix */);
#endif // VERBOSE
            ...
            )
        {
...
#if VERBOSE
                Debug.WriteLine("Block.Split Result: {0} {1}", this, newSplitBlock);
                Debug.WriteLine("Old block: {0}", this);
                this.DumpState(null /* no prefix */);
                Debug.WriteLine("New block: {0}", newSplitBlock);
            ...
            else
            {
                ...
                newSplitBlock = null;
#if VERBOSE
                Debug.WriteLine("Block.Split Result: all variables would be moved, so skipping");
                this.DumpState(null /* no prefix */);
#endif // VERBOSE
            }
...
#if VERBOSE
            Debug.WriteLine("GetConnectedVariables result:");
            foreach (var variable in lstVars) {
                Debug.WriteLine("  {0}", variable);
            }
#endif // VERBOSE
        }
