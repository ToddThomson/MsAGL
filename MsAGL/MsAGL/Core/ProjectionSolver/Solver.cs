...
namespace Microsoft.Msagl.Core.ProjectionSolver
{
    ...
    public class Solver
    {
        ...
        public Solution Solve(Parameters solverParameters)
        {
...
#if CACHE_STATS
            cacheStats.Print();
            Debug.WriteLine("  NumFinalBlocks = {0}, MinCacheBlocks = {1}, MaxCacheSize = {2}",
            ...
            Debug.WriteLine("Num final Blocks: {0}, Min Block Vars: {1}, Max Block Vars: {2}",
...
// ReSharper restore PossibleNullReferenceException
        ...
        private bool CheckForLimitsExceeded()
        {
            if (null != this.timeoutStopwatch)
            {
                ...
                {
#if VERBOSE
                    Debug.WriteLine("Solve() aborting due to time limit: max {0}, elapsed {1}",
            ...
            if (this.solverParams.OuterProjectIterationsLimit > 0)
            {
                ...
                {
#if VERBOSE
                    Debug.WriteLine("Solve() aborting due to max outer iterations: max {0}",
...
#if VERBOSE
            Debug.WriteLine("  -- End ProcessEqualityConstraints -- ");
#endif // VERBOSE
        }
        ...
        private bool Project()
        {
            ...
            {
#if VERBOSE
                Debug.WriteLine("Project() found no violations");
#endif // VERBOSE
                return false;
            }
            ...
            while (null != maxViolatedConstraint)
            {
...
#if VERBOSE
                Debug.WriteLine("MaxVio: {0}", maxViolatedConstraint);
                ...
                Debug.WriteLine("  most violated is: ({0}={1:F6})+{2:F0}<=({3}={4:F6})({5:F6})",
                ...
                if (this.solverParams.InnerProjectIterationsLimit > 0)
                {
                    ...
                    {
#if VERBOSE
                        Debug.WriteLine("PostProject aborting due to max inner iterations: max {0}",
        ...
        private void DumpCost()
        {
            ...
            Debug.WriteLine("NumBlocks = {0}, Cost = {1:F5}", allBlocks.Count, cost);
        }
#endif // VERBOSE
        ...
        private Block MergeBlocks(Constraint violatedConstraint)
        {
            ...
            if (blockFrom.Variables.Count > blockTo.Variables.Count)
            {
                ...
                distance = -distance;
#if VERBOSE
                Debug.WriteLine("MergeBlocks merging left ({0}) into right ({1}), distance = {2:F5}",
                                blockFrom, blockTo, distance);
            }
            else
            {
                Debug.WriteLine("MergeBlocks merging right ({0}) into left ({1}), distance = {2:F5}",
                                blockFrom, blockTo, distance);
#endif // VERBOSE
            }
#if VERBOSE
            Debug.WriteLine("blockFrom: {0}", blockFrom);
            blockFrom.DumpState(null /* no prefix */);
            Debug.WriteLine("blockTo: {0}", blockTo);
...
#if VERBOSE
            Debug.WriteLine("MergeBlocks result: {0}", blockTo);
            blockTo.DumpState(null /* no prefix */);
#endif // VERBOSE
        ...
        private bool SplitBlocks()
        {
            ...
            {
                Debug.WriteLine("SplitBlocks found nothing to split");
            }
#endif // VERBOSE
