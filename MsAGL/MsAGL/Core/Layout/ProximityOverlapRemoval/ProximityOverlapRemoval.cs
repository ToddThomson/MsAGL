...
#if DEBUG
//            if (DebugMode) {
//                Debug.WriteLine("Showing graph on startup.");
...
//            }
            ...
            Debug.WriteLine("Needed maxIterat: {0}", iter);
            Debug.WriteLine("BBox Area Ratio: {0}", boundingBoxArea/nodeBoxArea);
//            nodePositions = null;
//            nodeBoxes = null;
#if DEBUG && !SILVERLIGHT && !SHARPKIT
            if (DebugMode) {
        ...
        bool DoSingleIteration(int currentIteration, ref bool scanlinePhase) {
            ...
            if (currentIteration%10 == 0)
                Debug.WriteLine("Scanline: {0}, Crossings: {1}", scanlinePhase, numCrossings);
#endif
...
//#if DEBUG
//            Debug.WriteLine("AvgEdgeLength Scaling Method: {0}, ScaleFactor={1:F2}", Settings.InitialScaling, scaling);
//#endif
