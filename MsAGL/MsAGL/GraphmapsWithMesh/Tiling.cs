            ...
            {                
                for (int y = 1; y <= degList[x]; y++)
                {
                    for (int z = y + 1; z <= degList[x]; z++)
                        if (eList[x, y].nodeId == eList[x, z].nodeId) Debug.WriteLine("BAD");
                }
            }*/
                ...
                while (q.Count > 0)
                {
                    ...
                    for (int neighb = 1; neighb <= DegList[currentNode]; neighb++)
                    {
                        ...
                        if (VList[neighbor].Visited == false)
                        {
                            ...
                            //Debug.WriteLine(current_node   + "," +  vList[current_node].visited  + ":" +  neighbor  + "," +  vList[neighbor].visited  + "::" + (int)eList[current_node, neighb].weight); 
                ...
                foreach (var z in _sNet.V)
                {
                    ...
                    if (PointToSegmentDistance.GetDistance(w, VList[listNeighbors[i, 1]], z) < _edgeNodeSeparation[EList[w.Id, listNeighbors[i, 2]].Selected])
                    {
                        /*
                            ...
                            Debug.WriteLine(" " + w.Id + " " + ": " + VList[listNeighbors[i, 1]].Id + " " + z.Id + " " + PointToSegmentDistance.getDistance(w, VList[listNeighbors[i, 1]], z));
                            Debug.WriteLine(" " + w.XLoc + "," + w.YLoc +
                                    ": " + VList[listNeighbors[i, 1]].XLoc + " " + VList[listNeighbors[i, 1]].YLoc +
                                    ": " + z.XLoc + " " + z.YLoc +
                                    " " + PointToSegmentDistance.getDistance(w, VList[listNeighbors[i, 1]], z));
                            Debug.WriteLine("Distance = " + PointToSegmentDistance.getDistance(new Vertex(w.XLoc, w.YLoc), new Vertex(VList[listNeighbors[i, 1]].XLoc, VList[listNeighbors[i, 1]].YLoc), new Vertex(z.XLoc, z.YLoc)));
                            */
                        return false;
                    }
                }
        ...
        public bool IsWellSeperated(Vertex w, int w1, int w2, WeightedPoint[] pt, int numPoints)
        {
            ...
            for (int index = 1; index <= numPoints; index++)
            {
                ...
                */
                ...
                {
                    //Debug.WriteLine("close" + w.Id + " " + w1 + " " + w2 + " : " + pt[index].GridPoint);
                    return false;
                }
            }
            ...
            for (int neighb = 1; neighb <= DegList[w1]; neighb++)
            {
                ...
                {
                    //Debug.WriteLine("sharp*" + w.Id + " " + w2 + "(" + xDiff1 + "," + yDiff1 + ")" + w1 + "(" + xDiff2 + "," + xDiff2 + ")" + "::" + eList[w1, neighb].NodeId);
                    return false;
                }
            }
            ...
            for (int neighb = 1; neighb <= DegList[w2]; neighb++)
            {
                ...
                {
                    //Debug.WriteLine("sharp" + w.Id + " " + w2 + "(" + xDiff1 + "," + yDiff1 + ")" + w1 + "(" + xDiff2 + "," + xDiff2 + ")" + "::" + eList[w2, neighb].NodeId);
                    return false;
                }
            }
            //Debug.WriteLine( "Interesting : " + (Math.Atan2(1, 1)-Math.Atan2(1, -1)));
            return true;
        }
            ...
            while (localRefinementsFound && iteration > 0)
            {
                ...
                foreach (Vertex w in _sNet.V)
                {
                    ...
                    if (numNeighbors == 2)
                    {
                        ...
                        if (adjust)
                        {
                            //Debug.WriteLine(w.ID + " :: " + listNeighbors[2, 1] + " " + w1);
                            localRefinementsFound = true;
        ...
        public void MsaglDetour(Dictionary<int, Node> idToNode, bool omit)
        {
            ...
            for (int index = 0; index < N; index++)
            {
                ...
                for (neighbor = 0; neighbor < DegList[index]; neighbor++)
                {
                    ...
                    //Debug.WriteLine("Degenerate Case "+ w.XLoc + " " + w.YLoc + " : " +b.XLoc+" "+b.YLoc);
                }
    ...
    for (int i = 1; i <= num_points; i++)
    {
        ...
        for (int j = i + 1; j <= num_points; j++)
        {
            ...
            //if (p.Distance > 2 * eDist )
            {
                ...
                //Debug.WriteLine(pt[i].grid_point + " * " + pt[j].grid_point  );
