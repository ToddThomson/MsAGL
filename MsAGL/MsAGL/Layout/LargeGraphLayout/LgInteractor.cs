...
{
    /// <summary>
    /// Enables user interaction with a large graph.
    /// </summary>
    public class LgInteractor
    {
        ...
        public Tiling[] calculateGraphsForEachZoomLevel( Tiling g, Dictionary<Node, int> nodeToId, bool hugeGraph )
        {
            ...
            stopwatch.Start();
            Debug.WriteLine( "Calculating Graphs For Each ZoomLevel" );
                ...
                foreach ( Edge edge in _mainGeometryGraph.Edges )
                {
                    ...
                    if ( sourceZoomLevel <= i && targetZoomLevel <= i )
                    {
                        ...
                        if ( !ssp.ContainsKey( edge ) )
                        {
                            ...
                            if ( pathvertices.Count == 0 ) Debug.WriteLine( "missing path!" );
            ...
            stopwatch.Stop();
            Debug.WriteLine( "Graphs for all levels calculated = " + stopwatch.ElapsedMilliseconds );
            ...
            stopwatch.Stop();
            Debug.WriteLine( "Done Simplification = " + stopwatch.ElapsedMilliseconds );
        ...
        private static void ComputePathSimplification( Tiling g, Stopwatch stopwatch, List<Tiling> graphs )
        {
            ...
            Debug.WriteLine( "Starting Path Simplicfication" );
            stopwatch.Start();
        ...
        {
            Debug.WriteLine( "" );
            Debug.WriteLine( "Loading a huge graph? (Y/N)" );
            string input = ""; // FIXME Debug.ReadLine();
            return (input.StartsWith( "Y" ) || input.StartsWith( "y" ));
        }
        public bool PromptUserforFlow()
        {
            Debug.WriteLine( "" );
            Debug.WriteLine( "Use Flow to assign nodes on layers? (Y/N)" );
            string input = ""; // FIXME ""Console.ReadLine();
            return (input.StartsWith( "Y" ) || input.StartsWith( "y" ));
        }
        ...
        public void RunForMsaglFiles()
        {
            ...
            //ask user
            Debug.WriteLine( "Running in Default Settings - Enable UserPrompt in LgInteractor if needed." );
            ...
            stopwatch.Stop();
            Debug.WriteLine( "Time for DrawAtEachLevelQuotaBounded = " + stopwatch.ElapsedMilliseconds );
            Debug.WriteLine( "Total Number of Rails = " + _lgData.Levels[ _lgData.Levels.Count - 1 ].RailDictionary.Values.Count );
        ...
        public void DrawAtEachLevelQuotaBounded( Tiling[] g, Dictionary<Node, int> nodeToId )
        {
            ...
            Debug.WriteLine( "MAX Num of Level " + layer );
        ...
        public void DrawAtEachLevelQuotaSatisfied( Tiling[] g, Dictionary<Node, int> nodeToId )
        {
            ...
            foreach ( LgNodeInfo node in _lgData.SortedLgNodeInfos )
            {
                ...
                //Debug.WriteLine("vertex level " + node.GeometryNode + " : " + layer + " " + nodeOnlyZoomLevel+" " +g[graphLayer].VList[nodeToId[node.GeometryNode]].ZoomLevel);
        ...
        private Tiling TryCompetitionMeshApproach( out Dictionary<Node, int> nodeToId )
        {
            ...
            Debug.WriteLine( "Nodes = " + _mainGeometryGraph.Nodes.Count + "Edges = " + _mainGeometryGraph.Edges.Count );
            ...
            stopwatch.Stop();
            Debug.WriteLine( "Conected Graph and MDS Time = " + stopwatch.ElapsedMilliseconds );
            ...
            */
            ...
            MeshCreator.FastCompetitionMesh( g, idToNode, maxX, maxY, locationtoNode );
            stopwatch.Stop();
            Debug.WriteLine( "MeshCreation Time = " + stopwatch.ElapsedMilliseconds );
            ...
            stopwatch.Stop();
            Debug.WriteLine( "Detour Creation Time = " + stopwatch.ElapsedMilliseconds );
            ...
            //RouteByLayers();
            stopwatch.Stop();
            Debug.WriteLine( "Lev's = " + stopwatch.ElapsedMilliseconds );

            /*
            //UPDATE ACCORDING TO DIJKSTRA            
            Debug.WriteLine("Computing Edge Routes");
            stopwatch.Start();
            var g1 = ComputeEdgeRoutes(g, nodeToId, AdjacencyList);
            stopwatch.Stop();
            Debug.WriteLine("Edge Routing Time = " + stopwatch.ElapsedMilliseconds);               
            g = g1;
            */

            Debug.WriteLine( "Removing Deg 2 junctions" );
            //Remove Deg 2 Nodes when possible //less than a minute for 1500 vertices and 5000 edges
            stopwatch.Start();
            g.MsaglRemoveDeg2( idToNode );
            stopwatch.Stop();
            Debug.WriteLine( "Deg 2 removal Time = " + stopwatch.ElapsedMilliseconds );

            stopwatch.Start();
            PlanarGraphUtilities.TransformToGeometricPlanarGraph( g );
            stopwatch.Stop();
            Debug.WriteLine( "Planar Graph Building Time = " + stopwatch.ElapsedMilliseconds );

            stopwatch.Start();
            PlanarGraphUtilities.RemoveLongEdgesFromThinFaces( g );
            stopwatch.Stop();
            Debug.WriteLine( "Thin Face Removal Time = " + stopwatch.ElapsedMilliseconds );


            //Move the points towards median
            //Debug.WriteLine("Moving junctions to minimize ink");
            stopwatch.Start();
            ...
            stopwatch.Stop();
            Debug.WriteLine( "Ink Minimization Time = " + stopwatch.ElapsedMilliseconds );
        ...
        private void ComputeZoomLevelviaFlow( Dictionary<Node, int> nodeToId, int maxX, int maxY )
        {
            ...
            Debug.WriteLine( "Assign Nodes to zoomlevels via Flow. Increase delta for faster computation." );
            id = 0;
            buildTiles( 0, 0, maxX, maxY, 0, 0, 0 );
            Debug.WriteLine( "Number of tiles =" + id );
            ...
            Debug.WriteLine( "Total Nodes =" + g.N + " Nodes Found " + count );
            ...
            if ( flowfound )
                Debug.WriteLine( "A feasible flow found with " + maxdepth + " layers" );
            ...
            for ( int i = 0; i < maxtiles; i++ )
            {
                ...
                {
                    //Debug.WriteLine("prev,new" + g.VList[x].ZoomLevel + " " + (int)Math.Pow(2, tileDepth[i]));
            ...
            Debug.WriteLine("Number of edges in level 1 =" + id);
             * */
            /*
        ...
        public bool loadBipartiteData()
        {
            ...
            try
            {
                FileStream fileStream = new FileStream( line + ".set", FileMode.Open );
            ...
            catch ( Exception e )
            {
                ...
                Debug.WriteLine( "May not be a Bipartite Graph" );
                return false;
            }
        }
        public bool LoadNodeLocationsFromFile()
        {
            ...
            try
            {
                FileStream fileStream = new FileStream( line + ".loc", FileMode.Open );
                StreamReader file = new StreamReader( fileStream );
            ...
            {
                Debug.WriteLine( e.Message );
                Debug.WriteLine( "No prespecified location found." );
                return false;
            }
        ...
        private Tiling ComputeEdgeRoutes( Tiling g, Dictionary<Node, int> nodeId )
        {
            ...
            foreach ( var edge in _mainGeometryGraph.Edges )
            {
                ...
                if ( counter % 10000 == 0 ) Debug.WriteLine( "." );
        ...
        public bool MsaglNodePlottedandQuotaSatisfied( Tiling g, LgLevel level, int currentGraphLayer, int nodeToBePlotted, IEnumerable<Node> nodes, Dictionary<Node, int> nodeId )
        {
            ...
            //Debug.WriteLine("Total Rails = " + level.RailDictionary.Values.Count);
        ...
        public void Run()
        {
            ...
            LevelCalculator.RankGraph( _lgData, _mainGeometryGraph );
            LayoutTheWholeGraph();
#if !SILVERLIGHT && !SHARPKIT && !WINDOWS_UWP
            var timer = new Timer();
            timer.Start();
#endif
...
#if !SILVERLIGHT && !SHARPKIT && !WINDOWS_UWP
            timer.Stop();
            Debug.WriteLine("levels calculated for {0}", timer.Duration);
        ...
        void AddVisibleRailsAndNodes()
        {
            ...
            Debug.WriteLine( "current zoomlevel = " + level.ZoomLevel );
        ...
        */
            ...
            )
        {
            ...
            foreach ( LgNodeInfo s in nodes )
            {
                ...
                if ( ++numRouted % 100 == 1 ) Debug.Write( "." );
            }
        }
        ...
        public bool CheckSanityRoutes( LgNodeInfo s, int i )
        {
            ...
            if ( hasCycles ) Debug.WriteLine( "BAD SP Tree at: " + s );
            ...
            Point b, LgSkeletonLevel skeletonLevel )
        {
            ...
            foreach ( var rail in newIntersected )
            {
                ...
                if ( e == null )
                {
                    Debug.WriteLine( "rail {0} is not found in the vis graph", rail );
                }
            }
    ...
    nodesOnPrevLevel, new GridTraversal( bbox, iLevel ), _lgData.SkeletonLevels[ iLevel ].PathRouter );
        ...
        public void LabelingOfOneRun()
        {
            ...
            if ( numberOfLabeledNodes < _lgData.SortedLgNodeInfos.Count )
                Debug.WriteLine( "Failed to label {0} nodes", _lgData.SortedLgNodeInfos.Count - numberOfLabeledNodes );
        ...
        /*
                private void TestRouteNodeOverlaps(int i)
                {
                    ...
                    foreach (var tuple in skeletonLevel.EdgeTrajectories)
                    {
                        ...
                        if (ndsInt.Count > 2)
                        {
                            Debug.WriteLine("Path {0} -> {1} intersects nodes {2}", s.GeometryNode.ToString(), t.GeometryNode, ndsInt);
                        }
                    }
                }
        */
        ...
        void TestRailsForTrajectories( int iLevel )
        {
            foreach ( var path in _lgData.SkeletonLevels[ iLevel ].EdgeTrajectories.Values )
            {
                for ( int i = 0; i < path.Count - 1; i++ )
                {
                    ...
                    if ( rail == null )
                    {
                        Debug.WriteLine( "Rail not found for trajectory!" );
                    }
                }
            }
        }
        ...
        bool TestTrajectoriesPreserved( int iLevel )
        {
            ...
            foreach ( var st in _lgData.SkeletonLevels[ iLevel - 1 ].EdgeTrajectories )
            {
                ...
                if ( !newPath.First().Equals( ps ) )
                {
                    Debug.WriteLine( "Endpoint of old path not found on new path!" );
                    return false;
                }
                ...
                for ( i = 1, j = 1; i < oldPath.Count; i++, j++ )
                {
                    ...
                    {
                        Debug.WriteLine( "Point of old path not found on new path!" );
                        return false;
                    }
                    subdivEdge.Add( newPath[ j ] );
                    ...
                    Debug.WriteLine( "New path segment is not a line!" );
                    return false;
                }
            }
            return true;
        ...
        /*
                void TestTopRankedEdgeInfosOfTheRail(int iLevel) {
                    ...
                    foreach (Edge edge in _lgData.Levels[iLevel]._railsOfEdges.Keys) {
                        ...
                        if (!rails.Any()) {
                            Debug.WriteLine("Corrupt edge rail set");
                        }
                        foreach (Rail rail in rails) {
                            LgEdgeInfo ei = rail.TopRankedEdgeInfoOfTheRail;
                            if (ei == null) {
                                Debug.WriteLine("Corrupt Rail");
                            }

                            if (!_lgData.Levels[iLevel].RailDictionary.ContainsValue(rail)) {
                                Debug.WriteLine("used rail not in dictionary!");
                            }
                        }
                    }
                }
        ...
        /*
                bool TestEdgeConsistency(Edge edge) {
                    ...
                    if (ei.ZoomLevel != zoomLevel) {
                        Debug.WriteLine("ZoomLevel not set!");
                    }
        ...
        */
        ...
        public IEnumerable<Node> GetTileNodes( Tuple<int, int, int> tile )
        {
            ...
            return _lgData.Levels[ GetNumberOfLevels() - 1 ].NodeInfoTree.GetAllIntersecting( rect )
